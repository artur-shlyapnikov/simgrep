...
from typing import (
    ...
    Tuple,
    Any,
)  # Removed Dict as it's no longer directly used for aggregated_chunk_metadata
...
def search(
    ...
    if (
        chunk_embeddings.size == 0 or chunk_embeddings.shape[0] == 0
    ):  # Should be caught by earlier exit if no chunks
    ...
    if (
        not search_matches
    ):  # Handles case where all_chunkdata_objects might exist but no matches found
        console.print(
