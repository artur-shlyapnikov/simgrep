...
from simgrep.metadata_db import connect_persistent_db, MetadataDBError
from simgrep.exceptions import MetadataDBError as SimgrepMetadataDBError # Ensure consistent exception type
...
class TestPersistentMetadataDB:
    ...
    def test_connect_persistent_db_new_creation(self, persistent_db_path: pathlib.Path):
    ...
    def test_connect_persistent_db_existing_db(self, persistent_db_path: pathlib.Path):
        ...
        try:
            ...
            conn1.execute("INSERT INTO indexed_files (file_path, content_hash, file_size_bytes) VALUES (?, ?, ?)",
        ...
        try:
            ...
            count = conn2.execute("SELECT COUNT(*) FROM indexed_files WHERE file_path = '/test/file.txt';").fetchone()[0]
            assert count == 1
        finally:
            if conn2:
                conn2.close()

    def test_connect_persistent_db_directory_creation_failure(self, persistent_db_path: pathlib.Path):
        ...
        with pytest.raises(MetadataDBError, match=f"Could not create directory for database at {str(path_that_should_be_dir)}"):
            connect_persistent_db(persistent_db_path)

    def test_data_persistence_and_fk_constraint(self, persistent_db_path: pathlib.Path):
        ...
        try:
            ...
            content_hash1 = "abc"
            conn.execute("INSERT INTO indexed_files (file_path, content_hash, file_size_bytes, last_modified_os) VALUES (?, ?, ?, CURRENT_TIMESTAMP)",
            ...
            conn.execute("INSERT INTO text_chunks (file_id, usearch_label, chunk_text_snippet, start_char_offset, end_char_offset, token_count) VALUES (?, ?, ?, ?, ?, ?)",
                         [file1_id, 1001, "snippet1", 0, 10, 5])

            # Try inserting a chunk with a non-existent file_id (FK violation)
            with pytest.raises(duckdb.ConstraintException, match="FOREIGN KEY constraint failed"):
                 conn.execute("INSERT INTO text_chunks (file_id, usearch_label, chunk_text_snippet, start_char_offset, end_char_offset, token_count) VALUES (?, ?, ?, ?, ?, ?)",
                         [9999, 1002, "snippet2", 0, 10, 5]) # 9999 is a non-existent file_id

            # Try inserting duplicate file_path (UNIQUE constraint violation)
            with pytest.raises(duckdb.ConstraintException, match="UNIQUE constraint failed: indexed_files.file_path"):
                conn.execute("INSERT INTO indexed_files (file_path, content_hash, file_size_bytes) VALUES (?, ?, ?)",
                             [file_path1, "def", 2048]) # Same file_path1

            # Try inserting duplicate usearch_label (UNIQUE constraint violation)
            with pytest.raises(duckdb.ConstraintException, match="UNIQUE constraint failed: text_chunks.usearch_label"):
                conn.execute("INSERT INTO text_chunks (file_id, usearch_label, chunk_text_snippet, start_char_offset, end_char_offset, token_count) VALUES (?, ?, ?, ?, ?, ?)",
                             [file1_id, 1001, "snippet3", 10, 20, 6]) # Same usearch_label 1001
            conn.commit()
        finally:
            if conn:
                conn.close()
        ...
        try:
            ...
            file_count = conn_reopened.execute("SELECT COUNT(*) FROM indexed_files").fetchone()[0]
            chunk_count = conn_reopened.execute("SELECT COUNT(*) FROM text_chunks").fetchone()[0]
            assert file_count == 1
            assert chunk_count == 1

            retrieved_snippet = conn_reopened.execute("SELECT chunk_text_snippet FROM text_chunks WHERE usearch_label = ?", [1001]).fetchone()[0]
