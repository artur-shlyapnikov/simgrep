...
class TestLoadTokenizer:
    ...
    def test_load_valid_tokenizer(self):
    ...
    def test_load_invalid_tokenizer(self):
        from simgrep.processor import load_tokenizer
        with pytest.raises(RuntimeError, match=f"Failed to load tokenizer for model '{self.INVALID_MODEL_NAME}'"):
...
class TestChunkTextByTokens:
    ...
    def test_text_shorter_than_chunk_size(self, tokenizer):
        from simgrep.processor import chunk_text_by_tokens
    ...
    def test_text_equals_chunk_size(self, tokenizer):
        from simgrep.processor import chunk_text_by_tokens
    ...
    def test_multiple_chunks_no_overlap(self, tokenizer):
        from simgrep.processor import chunk_text_by_tokens
        ...
        expected_num_chunks = (num_tokens + chunk_size - 1) // chunk_size # Ceiling division
    ...
    def test_multiple_chunks_with_overlap(self, tokenizer):
        from simgrep.processor import chunk_text_by_tokens
        ...
        if len(chunks) > 1:
            ...
            if overlap_start_char < overlap_end_char:
                ...
                assert chunk2_text.startswith(text_overlap_from_chunk0[:len(chunk2_text)])


    def test_invalid_chunk_size(self, tokenizer):
        from simgrep.processor import chunk_text_by_tokens
    ...
    def test_invalid_overlap_size(self, tokenizer):
        from simgrep.processor import chunk_text_by_tokens
    ...
    def test_text_results_in_no_tokens(self, tokenizer):
        from simgrep.processor import chunk_text_by_tokens
...
class TestGenerateEmbeddings:
    ...
    def test_generate_valid_embeddings(self):
    ...
    def test_generate_embeddings_invalid_model(self):
        from simgrep.processor import generate_embeddings
        texts = ["This will fail"]
        with pytest.raises(RuntimeError, match=f"Failed to generate embeddings using model '{self.INVALID_MODEL_NAME}'"):
